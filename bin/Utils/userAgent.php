...
final class userAgent
{
    ...
    public static function getMobileOS(null|string $os = null): string
    {
        ...
        if (strpos($selected_os, ':androidDevice:') !== FALSE) {
            $selected_os = self::addAndroidDevice($selected_os);
        }
        if (strpos($selected_os, ':number') !== FALSE) {
            $selected_os = self::processRandomNumbers($selected_os);
        }
     ...
     * 
     * @param array|string $selected_os an OS
     * @return array|string|null
     */
    public static function processRandomNumbers(array|string $selected_os): array|string|null
    {
     ...
     * @return array|string|null
     */
    public static function processSpinSyntax(array|string $selected_os): array|string|null
    {
     ...
     * @return array|string|null
     */
    public static function processAndroidVersion(array|string $selected_os): array|string|null
    {
        self::$androidVersion = $version = self::$androidVersions[array_rand(self::$androidVersions)];
    ...
    public static function generate(devices|null $userAgent = NULL): string
    {
        ...
        if ($userAgent == 'chrome') {
            return 'Mozilla/5.0 (' . self::getOS($userAgent) . ') AppleWebKit/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)) . '.' . random_int(1, 50) . ' (KHTML, like Gecko) Chrome/' . self::chromeVersion([
                'min' => 47,
                'max' => 55
            ]) . ' Safari/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603));
        } elseif ($userAgent == 'firefox') {

            return 'Mozilla/5.0 (' . self::getOS($userAgent) . ') Gecko/' . (random_int(1, 100) > 30 ? '20100101' : '20130401') . ' Firefox/' . self::firefoxVersion([
                'min' => 45,
                'max' => 74
            ]);
        } elseif ($userAgent == 'explorer') {

            return 'Mozilla / 5.0 (compatible; MSIE ' . ($int = random_int(7, 11)) . '.0; ' . self::getOS('windows') . ' Trident / ' . ($int == 7 || $int == 8 ? '4' : ($int == 9 ? '5' : ($int == 10 ? '6' : '7'))) . '.0)';
        } elseif ($userAgent == 'mobile' || $userAgent == 'android' || $userAgent == 'iphone' || $userAgent == 'ipad' || $userAgent == 'ipod') {

            return 'Mozilla/5.0 (' . self::getMobileOS($userAgent) . ') AppleWebKit/' . (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)) . '.' . random_int(1, 50) . ' (KHTML, like Gecko)  Chrome/' . self::chromeVersion([
                'min' => 47,
                'max' => 55
